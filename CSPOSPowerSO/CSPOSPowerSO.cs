...
namespace OpenPOS.CSSO
{
    ...
    [ComVisible(true)]
    [Guid("BE59CD25-8DD3-4773-9F70-8A28F082D6AE")]
    [ClassInterface(ClassInterfaceType.None)]
    [ProgId("OPOS.POSPower.OpenPOS.CSSO.CSPOSPowerSO.1")]
    public class CSPOSPowerSO : IOPOSPOSPowerSO, IDisposable
    {
        ...
        internal int _enforcedShutdownDelayTime;
        internal int _powerFailDelayTime;
        internal bool _quickChargeMode;
        internal int _quickChargeTime;
        internal int _upsChargeState;
        internal int _batteryCapacityRemaining;
        internal int _batteryCriticallyLowThreshold;
        internal int _batteryLowThreshold;
        internal int _powerSource;

        internal int _batteryCapacityRemainingInSeconds;
        internal int _batteryCriticallyLowThresholdInSeconds;
        internal int _batteryLowThresholdInSeconds;
        internal int _chargeTime;

        internal bool _capFanAlarm;
        internal bool _capHeatAlarm;
        internal bool _capQuickCharge;
        internal bool _capShutdownPOS;
        internal int _capUPSChargeState;
        internal bool _capBatteryCapacityRemaining;
        internal bool _capRestartPOS;
        internal bool _capStandbyPOS;
        internal bool _capSuspendPOS;
        internal bool _capVariableBatteryCriticallyLowThreshold;
        internal bool _capVariableBatteryLowThreshold;

        internal bool _capBatteryCapacityRemainingInSeconds;
        internal bool _capVariableBatteryCriticallyLowThresholdInSeconds;
        internal bool _capVariableBatteryLowThresholdInSeconds;
        internal bool _capChargeTime;
        ...
        public CSPOSPowerSO()
        {
            ...
            _enforcedShutdownDelayTime = 0;
            _powerFailDelayTime = 0;
            _quickChargeMode = false;
            _quickChargeTime = 0;
            _upsChargeState = 0;
            _batteryCapacityRemaining = 0;
            _batteryCriticallyLowThreshold = 0;
            _batteryLowThreshold = 0;
            _powerSource = 0;

            _batteryCapacityRemainingInSeconds = 0;
            _batteryCriticallyLowThresholdInSeconds = 0;
            _batteryLowThresholdInSeconds = 0;
            _chargeTime = 0;

            _capFanAlarm = false;
            _capHeatAlarm = false;
            _capQuickCharge = false;
            _capShutdownPOS = false;
            _capUPSChargeState = 0;
            _capBatteryCapacityRemaining = false;
            _capRestartPOS = false;
            _capStandbyPOS = false;
            _capSuspendPOS = false;
            _capVariableBatteryCriticallyLowThreshold = false;
            _capVariableBatteryLowThreshold = false;

            _capBatteryCapacityRemainingInSeconds = false;
            _capVariableBatteryCriticallyLowThresholdInSeconds = false;
            _capVariableBatteryLowThresholdInSeconds = false;
            _capChargeTime = false;
        ...
        {
            if (registerType != typeof(CSPOSPowerSO))
            {
                return;
            }
        ...
        public int GetPropertyNumber(int PropIndex)
        {
            ...
            switch (PropIndex)
            {
                ...
                case (int)OPOSPOSPowerInternals.PIDXPwr_EnforcedShutdownDelayTime:
                    value = _enforcedShutdownDelayTime;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_PowerFailDelayTime:
                    value = _powerFailDelayTime;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_QuickChargeMode:
                    value = Convert.ToInt32(_quickChargeMode);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_QuickChargeTime:
                    value = _quickChargeTime;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_UPSChargeState:
                    value = _upsChargeState;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryCapacityRemaining:
                    value = _batteryCapacityRemaining;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryCriticallyLowThreshold:
                    value = _batteryCriticallyLowThreshold;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryLowThreshold:
                    value = _batteryLowThreshold;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_PowerSource:
                    value = _powerSource;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryCapacityRemainingInSeconds:
                    value = _batteryCapacityRemainingInSeconds;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryCriticallyLowThresholdInSeconds:
                    value = _batteryCriticallyLowThresholdInSeconds;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryLowThresholdInSeconds:
                    value = _batteryLowThresholdInSeconds;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_ChargeTime:
                    value = _chargeTime;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapFanAlarm:
                    value = Convert.ToInt32(_capFanAlarm);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapHeatAlarm:
                    value = Convert.ToInt32(_capHeatAlarm);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapQuickCharge:
                    value = Convert.ToInt32(_capQuickCharge);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapShutdownPOS:
                    value = Convert.ToInt32(_capShutdownPOS);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapUPSChargeState:
                    value = _capUPSChargeState;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapBatteryCapacityRemaining:
                    value = Convert.ToInt32(_capBatteryCapacityRemaining);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapRestartPOS:
                    value = Convert.ToInt32(_capRestartPOS);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapStandbyPOS:
                    value = Convert.ToInt32(_capStandbyPOS);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapSuspendPOS:
                    value = Convert.ToInt32(_capSuspendPOS);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapVariableBatteryCriticallyLowThreshold:
                    value = Convert.ToInt32(_capVariableBatteryCriticallyLowThreshold);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapVariableBatteryLowThreshold:
                    value = Convert.ToInt32(_capVariableBatteryLowThreshold);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapBatteryCapacityRemainingInSeconds:
                    value = Convert.ToInt32(_capBatteryCapacityRemainingInSeconds);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapVariableBatteryCriticallyLowThresholdInSeconds:
                    value = Convert.ToInt32(_capVariableBatteryCriticallyLowThresholdInSeconds);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapVariableBatteryLowThresholdInSeconds:
                    value = Convert.ToInt32(_capVariableBatteryLowThresholdInSeconds);
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_CapChargeTime:
                    value = Convert.ToInt32(_capChargeTime);
                    break;
            }
        ...
        public void SetPropertyNumber(int PropIndex, int Number)
        {
            ...
            switch (PropIndex)
            {
                ...
                case (int)OPOSPOSPowerInternals.PIDXPwr_EnforcedShutdownDelayTime:
                    _enforcedShutdownDelayTime = Number;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryCriticallyLowThreshold:
                    _batteryCriticallyLowThreshold = Number;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryLowThreshold:
                    _batteryLowThreshold = Number;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryCriticallyLowThresholdInSeconds:
                    _batteryCriticallyLowThresholdInSeconds = Number;
                    break;

                case (int)OPOSPOSPowerInternals.PIDXPwr_BatteryLowThresholdInSeconds:
                    _batteryLowThresholdInSeconds = Number;
                    break;
            }
        ...
        public int StandbyPOS(int Reason)
        {
            _resultCode = (int)OPOS_Constants.OPOS_E_ILLEGAL;
            _resultCodeExtended = 0;
            return _resultCode;
        ...
        public int SuspendPOS(int Reason)
        {
            _resultCode = (int)OPOS_Constants.OPOS_E_ILLEGAL;
            _resultCodeExtended = 0;
            return _resultCode;
