...
namespace OpenPOS.CSSO
{
    ...
    [ComVisible(false)]
    internal class OposEventIndividualRecognition : OposEvent
    {
        internal string _individualIDs;
        internal string _individualRecognitionInformation;

        #region IDisposable Support

        public OposEventIndividualRecognition()
        {
            _individualIDs = string.Empty;
            _individualRecognitionInformation = string.Empty;
        }
        ...
        protected override void Dispose(bool disposing)
        {
            try
            {
                if (!this._disposedValue)
                {
                    ...
                    {
                        _individualIDs = null;
                        _individualRecognitionInformation = null;
                    }
    ...
    [ComVisible(true)]
    [Guid("18BEC181-D053-4952-BD18-E69887B3A8C0")]
    [ClassInterface(ClassInterfaceType.None)]
    [ProgId("OPOS.IndividualRecognition.OpenPOS.CSSO.CSIndividualRecognitionSO.1")]
    public class CSIndividualRecognitionSO : IOPOSIndividualRecognitionSO, IDisposable
    {
        ...
        internal string _capIndividualList;
        internal volatile string _individualIDs;
        internal string _individualRecognitionFilter;
        internal volatile string _individualRecognitionInformation;
        ...
        internal volatile List<OposEventIndividualRecognition> _oposEvents;
        internal volatile bool _killEventThread;

        #region IDisposable Support Constructer / Destructer

        public CSIndividualRecognitionSO()
        {
            ...
            _capIndividualList = string.Empty;
            _individualIDs = string.Empty;
            _individualRecognitionFilter = string.Empty;
            _individualRecognitionInformation = string.Empty;
            ...
            _oposEvents = new List<OposEventIndividualRecognition>();
        ...
        {
            if (registerType != typeof(CSIndividualRecognitionSO))
            {
                return;
            }
        ...
        public string GetPropertyString(int PropIndex)
        {
            ...
            switch (PropIndex)
            {
                ...
                case (int)OPOSIndividualRecognitionInternals.PIDXIrcg_CapIndividualList:
                    value = string.Copy(_capIndividualList);
                    break;

                case (int)OPOSIndividualRecognitionInternals.PIDXIrcg_IndividualIDs:
                    lock (_dataAccessLock)
                    {
                        value = string.Copy(_individualIDs);
                    }
                    break;
                case (int)OPOSIndividualRecognitionInternals.PIDXIrcg_IndividualRecognitionFilter:
                    value = string.Copy(_individualRecognitionFilter);
                    break;
                case (int)OPOSIndividualRecognitionInternals.PIDXIrcg_IndividualRecognitionInformation:
                    lock (_dataAccessLock)
                    {
                        value = string.Copy(_individualRecognitionInformation);
                    }
                    break;
            }
        ...
        public void SetPropertyString(int PropIndex, string String)
        {
            ...
            {
                case (int)OPOSIndividualRecognitionInternals.PIDXIrcg_IndividualRecognitionFilter:
                    _individualRecognitionFilter = String;
                    break;
            }
        ...
        internal void DoEventThread()
        {
            while (_killEventThread == false)
            {
                 ...
                 && (FindNotifiableEvent()))
                {
                    ...
                    OposEventIndividualRecognition individualRecognitionEvent = _oposEvents[0];
                    _oposEvents.RemoveAt(0);

                    _eventListMutex.ReleaseMutex();

                    switch (individualRecognitionEvent.eventType)
                    {
                        case OposEvent.EVENT_DATA:
                            lock (_dataAccessLock)
                            {
                                _individualIDs = individualRecognitionEvent._individualIDs;
                                _individualRecognitionInformation = individualRecognitionEvent._individualRecognitionInformation;
                                _dataEventEnabled = false;
                            }
                            _oposCO.SOData(individualRecognitionEvent.status);
                            break;
                        case OposEvent.EVENT_DIRECTIO:
                            _oposCO.SODirectIO(individualRecognitionEvent.eventNumber, ref individualRecognitionEvent.pData, ref individualRecognitionEvent.pString);
                            // todo:somethingelse
                            break;
                        case OposEvent.EVENT_ERROR:
                            _oposCO.SOError(individualRecognitionEvent.resultCode, individualRecognitionEvent.resultCodeExtended, individualRecognitionEvent.errorLocus, ref individualRecognitionEvent.pErrorResponse);
                            if (individualRecognitionEvent.pErrorResponse == (int)OPOS_Constants.OPOS_ER_CLEAR)
                            {
                        ...
                        case OposEvent.EVENT_STATUSUPDATE:
                            _oposCO.SOStatusUpdate(individualRecognitionEvent.data);
                            break;
                    }
                }
                else
        ...
        internal void ClearEventList(int flags)
        {
            if (_eventListMutex.WaitOne(OposEvent.EVENT_MUTEX_TIMEOUT))
            {
                for (int i = _oposEvents.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        OposEventIndividualRecognition e = _oposEvents[i];
