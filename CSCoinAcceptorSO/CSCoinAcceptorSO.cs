...
namespace OpenPOS.CSSO
{
    ...
    [ComVisible(true)]
    [Guid("AB9E3CAC-1D21-4E7A-A593-034D3FC2A505")]
    [ClassInterface(ClassInterfaceType.None)]
    [ProgId("OPOS.CoinAcceptor.OpenPOS.CSSO.CSCoinAcceptorSO.1")]
    public class CSCoinAcceptorSO : IOPOSCoinAcceptorSO, IDisposable
    {
        ...
        public CSCoinAcceptorSO()
        {
            ...
            _depositAmount = 0;
            _depositStatus = (int)OPOSCoinAcceptorConstants.CACC_STATUS_DEPOSIT_END;
            _fullStatus = (int)OPOSCoinAcceptorConstants.CACC_STATUS_OK;
        ...
        {
            if (registerType != typeof(CSCoinAcceptorSO))
            {
                return;
            }
        ...
        public int GetPropertyNumber(int PropIndex)
        {
            ...
            switch (PropIndex)
            {
                ...
                case (int)OPOSCoinAcceptorInternals.PIDXCacc_DepositAmount:
                    value = _depositAmount;
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_DepositStatus:
                    value = _depositStatus;
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_FullStatus:
                    value = _fullStatus;
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_RealTimeDataEnabled:
                    value = Convert.ToInt32(_realTimeDataEnabled);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_CapDiscrepancy:
                    value = Convert.ToInt32(_capDiscrepancy);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_CapFullSensor:
                    value = Convert.ToInt32(_capFullSensor);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_CapJamSensor:
                    value = Convert.ToInt32(_capJamSensor);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_CapNearFullSensor:
                    value = Convert.ToInt32(_capNearFullSensor);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_CapPauseDeposit:
                    value = Convert.ToInt32(_capPauseDeposit);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_CapRealTimeData:
        ...
        public string GetPropertyString(int PropIndex)
        {
            ...
            switch (PropIndex)
            {
                ...
                case (int)OPOSCoinAcceptorInternals.PIDXCacc_CurrencyCode:
                    value = string.Copy(_currencyCode);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_DepositCashList:
                    value = string.Copy(_depositCashList);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_DepositCodeList:
                    value = string.Copy(_depositCodeList);
                    break;

                case (int)OPOSCoinAcceptorInternals.PIDXCacc_DepositCounts:
