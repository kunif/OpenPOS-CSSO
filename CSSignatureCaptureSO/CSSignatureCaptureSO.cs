...
namespace OpenPOS.CSSO
{
    ...
    [ComVisible(true)]
    [Guid("1B9BF276-CB22-425C-91E8-C6ACCBCE07C3")]
    [ClassInterface(ClassInterfaceType.None)]
    [ProgId("OPOS.SignatureCapture.OpenPOS.CSSO.CSSignatureCaptureSO.1")]
    public class CSSignatureCaptureSO : IOPOSSignatureCaptureSO, IDisposable
    {
        ...
        internal bool _capDisplay;
        internal bool _capUserTerminated;
        internal bool _capRealTimeData;

        internal byte[] _rawData;
        internal byte[] _pointArray;
        ...
        public CSSignatureCaptureSO()
        {
            ...
            _capDisplay = false;
            _capUserTerminated = false;
            _capRealTimeData = false;

            _rawData = new byte[0];
            _pointArray = new byte[0];
        ...
        {
            if (registerType != typeof(CSSignatureCaptureSO))
            {
                return;
            }
        ...
        public int GetPropertyNumber(int PropIndex)
        {
            ...
            switch (PropIndex)
            {
                ...
                case (int)OPOSSigCapInternals.PIDXSig_MaximumX:
                    value = _maximumX;
                    break;

                case (int)OPOSSigCapInternals.PIDXSig_MaximumY:
                    value = _maximumY;
                    break;

                case (int)OPOSSigCapInternals.PIDXSig_TotalPoints:
                    value = _totalPoints;
                    break;

                case (int)OPOSSigCapInternals.PIDXSig_RealTimeDataEnabled:
                    value = Convert.ToInt32(_realTimeDataEnabled);
                    break;

                case (int)OPOSSigCapInternals.PIDXSig_CapDisplay:
                    value = Convert.ToInt32(_capDisplay);
                    break;

                case (int)OPOSSigCapInternals.PIDXSig_CapUserTerminated:
                    value = Convert.ToInt32(_capUserTerminated);
                    break;

                case (int)OPOSSigCapInternals.PIDXSig_CapRealTimeData:
        ...
        public string GetPropertyString(int PropIndex)
        {
            ...
            switch (PropIndex)
            {
                ...
                case (int)OPOSSigCapInternals.PIDXSig_RawData:
                    value = SOCommon.ToStringFromByteArray(_rawData, _binaryConversion);
                    break;

                case (int)OPOSSigCapInternals.PIDXSig_PointArray:
                    value = SOCommon.ToStringFromByteArray(_pointArray, _binaryConversion);
                    break;
            }
        ...
        public int BeginCapture(string FormName)
        {
            _resultCode = (int)OPOS_Constants.OPOS_E_ILLEGAL;
            _resultCodeExtended = 0;
            return _resultCode;
